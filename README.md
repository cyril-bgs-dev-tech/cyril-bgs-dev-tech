<div align="center">

# 👋 Cyril BOURGEOIS

### Data Scientist · MLOps — Détection d'anomalies temps réel

[![Portfolio](https://img.shields.io/badge/Portfolio-visiter%20le%20site-2563eb?style=for-the-badge&logo=quarto)](https://cyril-bgs-dev-tech.github.io/Portfolio/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/cyril-bourgeois/)
[![Kaggle](https://img.shields.io/badge/Kaggle-Top%201%25-20BEFF?style=for-the-badge&logo=kaggle)](https://www.kaggle.com/cyrilbourgeois)

</div>

Reconversion armée → commerce → data science. Je construis des systèmes ML **de bout en bout** : de la reproduction rigoureuse de l'état de l'art jusqu'à la mise en production observable.

## 🔭 En ce moment

- ⚙️ **Vigilance** — plateforme de détection d'anomalies temps réel : DAG event-driven 9 workers (Redis, Docker), 3 canaux de détection décorrélés, cycle de vie d'alarme en épisode unique, cockpit Streamlit 40+ pages ([voir la fiche projet](https://cyril-bgs-dev-tech.github.io/Portfolio/projets_MLOps_Vigilancy/README.html))
- 🔬 **Benchmarks de reproduction AD** — ADBench (tabulaire) & TSB-AD (séries temporelles) : reproduire exactement les scores publiés avant de prétendre les battre — VUS-PR, point-adjust banni, validation LODO
- 🏆 **Kaggle** — 35+ compétitions, 4 top 1% mondial (meilleur rang : #13/1757)

## 🛠️ Stack

`Python` `Scikit-learn` `XGBoost` `PyTorch` `Polars` `Redis` `Docker` `Prometheus` `Grafana` `Streamlit` `SHAP`

---

<div align="center">

📫 **cyril.bgs.dev.tech@gmail.com** · 🌐 [cyril-bgs-dev-tech.github.io/Portfolio](https://cyril-bgs-dev-tech.github.io/Portfolio/)

</div>
